...
export function KpiCard({ label, value, icon: Icon, trend, description, type = 'neutral', className }: KpiCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "bg-white border border-[#E5E7EB] p-6 rounded-[32px] shadow-sm space-y-4 hover:shadow-md transition-all group",
        type !== 'neutral' && typeStyles[type],
        className
      )}
    >
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        colors[insight.type as keyof typeof colors] || colors.info
      )}
    >
      <div className="text-2xl mt-1">{icons[insight.type as keyof typeof icons]}</div>
      <div className="space-y-1">
