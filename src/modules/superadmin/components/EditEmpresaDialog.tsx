...
export interface EditEmpresaFormData {
  nome: string;
  cnpj: string;
  plano: string;
}

export interface EditEmpresaDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  empresa: EmpresaWithUserCount | null;
  onSubmit: (data: EditEmpresaFormData) => Promise<void>;
...
export function EditEmpresaDialog({
  ...
  useEffect(() => {
    if (empresa && open) {
      ...
      setEditPlano(empresa.plano || 'standard');
    }
  }, [empresa, open]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader className="border-b border-slate-800 pb-3 flex flex-row items-center gap-2.5 space-y-0">
          ...
          <div>
            <DialogTitle className="text-sm font-black text-white">Editar Dados da Empresa</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-800">
            ...
            <button
              ...
              className="px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition-all shadow-md shadow-blue-600/30 flex items-center gap-2 disabled:opacity-50 cursor-pointer"
            >
              {isSubmitting ? (
                <>
