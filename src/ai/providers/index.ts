/**
 * DDSulf Operational AI Provider Wrapper
...
export class AIProviderService {
  ...
  private static generateOfflineHeuristicAnswer(prompt: string, context: SystemCoreContext): string {
    ...
    if (p.includes('olá') || p.includes('oi') || p.includes('bom dia') || p.includes('boa tarde')) {
      return `Olá, **${context.userName}**! Sou o assistente operacional DDSulf de contingência. No momento, operando em modo **Offline Resiliente**. Como posso ajudar na retaguarda operacional?`;
    }
    ...
    if (isTechnical && (p.includes('custo') || p.includes('margem') || p.includes('lucro') || p.includes('preço') || p.includes('faturamento'))) {
      return `### 🔒 Acesso Restrito por Governança\n\nOlá **${context.userName}**, notamos uma consulta de teor financeiro. De acordo com as diretrizes do seu perfil do tipo **${context.activeRole.toUpperCase()}**, os custos unitários e as margens de faturamento comercial do DDSulf são blindadas.\n\n*Para visualizar ou editar custos operacionais de defensivos, faça login com uma conta Admin ou Financeira.*`;
    }
